...
const {
  createOrderFulfiller,
} = require("../../otherServices/orderFulfillers.service");
    ...
    create(orderBody, (err, results) => {
      ...
      createShippingAddress(shippingBody, (shippingErr, shippingRes) => {
        ...
        getFulfillersCount((countErr, countResults) => {
          ...
          createOrderFulfiller(
            orderFulfillerData,
            (orderFulfillerErr, orderFulfillerRes) => {
    ...
    getOrders((err, results) => {
      ...
      getOrderItems((err, itemResults) => {
        ...
        for (let i = 0; i < results.length; i++) {
          const products = itemResults.filter(
            (item) => item.order_id === results[i].id
          );
    ...
    getUserOrders(addr, (err, results) => {
      ...
      async.eachOf(
        ...
        (result, index, callBack) => {
          getItemsByOrderId(result.order_id, (itemErr, itemResult) => {
            if (itemErr) {
              return callBack(itemErr);
            }
